...
def main():
    ...
    if len(args.train_file_name.split(',')) == 1:
         ...
         for x in train_temp:
             if args.prefix:
                for y in x.split('\n'):
                    train_list.append('<|endoftext|> <|context|> ' + '<dst> <' + temp_language + '> <' + temp_language + '>: ' + y.split('<|context|>')[1])
             else:
                train_list += x.split('\n')
    else:
         train_list = []
         for x in args.train_file_name.split(','):
             ...
             for y in train_temp:
                 for z in y.split('\n'):
                    train_list.append('<|endoftext|> <|context|> ' + '<dst> <' + temp_language + '> <'+temp_language+'>: ' + z.split('<|context|>')[1])
    ...
    test_list1 = open(args.test_file_name, "r", encoding='utf-8').read().split('\n\n')[0:-1]
    ...
    for data in test_temp:
        if args.prefix:
            for x in data.split('\n'):
                ...
                test_list2.append(
                     '<|endoftext|> <|context|> ' + '<dst> <' + temp_language + '> <' + temp_language + '>: ' +x.split('<|context|>')[1]
                     )
        else:
            test_list2 += data.split('\n')
    # 开始训练
    ...
    if args.mode == 'train':
        ...
        if args.local_rank == 0:
            ...
            if args.mix_train:
                ...
                result = evaluate_avg_acc(model, device, test_list2, args, tokenizer, tb_writer, overstep)
    elif args.mode == 'eva':
        best_model = ''
        if args.eval_all_checkpoints:
            checkpoints = [args.dialogue_model_output_path + c for c in
