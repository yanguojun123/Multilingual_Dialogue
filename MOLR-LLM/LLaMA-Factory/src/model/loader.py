...
def load_model_and_tokenizer(
    ...
    ## PP added
    if model_args.molr_exp=='mix': # Molr__LLM
        print('mix')
        from .molr import TripleMolrCausalLM
        model = TripleMolrCausalLM.from_pretrained(
            model_args.model_name_or_path,
            config=config,
    ...
    elif model_args.molr_exp=='mix0':
        print('mix0')
        from .molr import TripleMolrConditionalCausalLM
        model = TripleMolrConditionalCausalLM.from_pretrained(
            model_args.model_name_or_path,
            config=config,
            torch_dtype=model_args.compute_dtype,
            low_cpu_mem_usage=(not is_deepspeed_zero3_enabled()),
